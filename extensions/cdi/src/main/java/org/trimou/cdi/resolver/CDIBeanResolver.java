...
public class CDIBeanResolver extends AbstractResolver {
    ...
    public Object resolve(Object contextObject, String name, ResolutionContext context) {
        ...
        Bean<?> bean = beanCache.get(name).orElse(null);
        if (bean == null) {
            // Unsuccessful lookup already performed
            return null;
        }
        return getReference(bean, context);
    }
    ...
    private <T> Object getReference(Bean<T> bean, ResolutionContext context) {
        ...
        if (Dependent.class.equals(bean.getScope())) {
            ...
            T reference = (T) beanManager.getReference(bean, Object.class, creationalContext);
            context.registerReleaseCallback(() -> bean.destroy(reference, creationalContext));
            return reference;
        } else {
    ...
    private class CDIBeanHint implements Hint {
        ...
        @Override
        public Object resolve(Object contextObject, String name, ResolutionContext context) {
            return contextObject != null ? null : CDIBeanResolver.this.getReference(bean, context);
        }
