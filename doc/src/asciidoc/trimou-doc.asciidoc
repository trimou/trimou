...
** this metadata has some useful properties: +iter.index+, +iter.hasNext+, +iter.isFirst+ and +iter.isLast+,
...
+org.trimou.engine.cache.ComputingCache+ is a simple abstraction for thread-safe computing (lazy loading) cache. It's used in some internal components (e.g. +ReflectionResolver+) and may also be used in custom components too.
+org.trimou.engine.cache.ComputingCacheFactory+ component is responsible for creating new instances of +ComputingCache+. The default computing cache implementation is backed by +com.google.common.cache.LoadingCache+.
