...
Trimou is yet another https://github.com/mustache[Mustache] implementation written in Java. It's goal is to provide a *simple to use* and *easy to extend* templating engine for any Java SE or Java EE application. So far there are two ready-to-use extensions, <<cdi,trimou-extension-cdi>> and <<servlets,trimou-extension-servlet>>. Both provide (among others) a special <<resolver,resolver>> to extend the set of resolvable objects for your templates, or lookup CDI beans and Servlet request wrapper respectively.
...
* for any context object Trimou tries to find and invoke a read method (public, non-static, with no params) with name +key+, +getKey+ or +isKey+ defined on the context object's class and its superclasses (except +java.lang.Object+),
...
<2> If "foo" is an instance of Map, get the value of key "bar"; otherwise try to invoke bar(), getBar() or isBar() on the "foo" instance
...
<7> true if the iteration has more elements
...
You can implement +org.trimou.api.Lambda+ interface in order to define a lambda/callable object. Predefined abstract +org.trimou.lambda.SpecCompliantLambda+ follows the behaviour defined by the spec:
...
See +org.trimou.api.Lambda+ API javadoc for more info.
...
Mustache mustache = engine.compile("myTemplateName", "{{! empty template}}"); <2>
...
String output = MustacheEngineBuilder
                  .newBuilder()
                  .build()
                  .compile("myTemplateName", "{{! empty template}}")
                  .render(null);
----
<1> Build the engine
...
Mustache mustache = engine.compile("fooTemplate", "{{foo}}");
...
* Add global lambdas/callable objects; see <<lambdas,why>>
...
MustacheEngine engine = MustacheEngineBuilder
                            .newBuilder()
                            .addGlobalLambda(mySuperUsefulLambdaInstance)
                            .build();
----
...
MustacheEngine engine = MustacheEngineBuilder
                           ...
                           .build();
Mustache mustache = engine.get("foo"); <2>
...
Trimou has a very basic i18n support. Basically it provides three optional resolvers: +org.trimou.engine.resolver.i18n.NumberFormatResolver+, +org.trimou.engine.resolver.i18n.DateTimeFormatResolver+, +org.trimou.engine.resolver.i18n.ResourceBundleResolver+ and one optional lambda: +org.trimou.lambda.i18n.ResourceBundleLambda+. All these components rely  on +org.trimou.spi.engine.LocaleSupport+ implementation to get the current +Locale+. See javadoc for more info.
...
MustacheEngine engine = MustacheEngineBuilder
                           ...
                           .build();
Mustache mustache = engine.get("foo");
...
SPI: +org.trimou.spi.engine.Resolver+
...
All resolvers have a priority and resolvers with *higher priority* are called *first*. Keep in mind that all resolvers must be thread-safe. There are two ways to extend the basic set of resolvers. Trimou utilizes JDK http://docs.oracle.com/javase/6/docs/api/java/util/ServiceLoader.html[service-provider loading facility]. Simply create a file with name +org.trimou.spi.engine.Resolver+, write down FCQN of your resolver (one per line) and place the file into +META-INF/services+ folder and the listed resolvers are loaded automatically. This way of loading resolvers may be disabled per engine, see also +MustacheEngineBuilder.omitServiceLoaderResolvers()+.
...
SPI: +org.trimou.spi.engine.TemplateLocator+
...
SPI: +org.trimou.spi.engine.TextSupport+
...
SPI: +org.trimou.spi.engine.LocaleSupport+
...
Intercepts CDI bean business method invocation and returns the rendered template. The method return type must be of type +java.lang.Object+. If the method code returns an instance of +java.util.Map+, the return value is passed to the compiled template.

[source,java]
----
  @Rendered(template = "foo.html") <1>
  public Object ping(String value) {
    return ImmutableMap.<String, Object> of("foo", value); <2>
  }
----
<1> Use @Rendered interceptor binding to associate RenderedInterceptor and specify the template name
<2> This return value is passed to the compiled template
