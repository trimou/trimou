...
Since version 1.5.0 handlebars-like helpers are supported. There are four helpers registered automatically: +if+, +unless+, +each+, +with+ (see http://handlebarsjs.com for examples :-). Other helpers may be registered via +MustacheEngineBuilder.registerHelper(String, Helper)+. See also <<custom_helpers,Helper>> section.
...
NOTE: Instances of +MustacheEngineBuilder+ are not reusable. The builder is considered immutable once the
 +build()+ method is called - subsequent invocations of some +MustacheEngineBuilder+ methods result in +IllegalStateException+.
...
You can set the custom +org.trimou.engine.text.TextSupport+ instance with +org.trimou.engine.MustacheEngineBuilder.setTextSupport()+ method. So far there's only one method to implement - +escapeHtml(String)+ (see also <<escaping_hml>>). Implement your own logic to extend functionality or improve performance.
...
+org.trimou.handlebars.Helper+ API is inspired by Handlebars but it's not 100% compatible. Mainly it does not define "inverse" section, so for example +if+ helper doesn't support +else+ block. On the other hand the helper is able to validate the tag definition (see +Helper.validate(HelperDefinition)+) and fail fast if there's invalid number of arguments etc.

Helper may be registered via +MustacheEngineBuilder.registerHelper(String, Helper)+. Each helper must be registered with a unique name. If there are more helpers registered with the same name an +IllegalArgumentException+ is thrown during engine build. 

NOTE: Trimou provides some useful helpers which are not automatically registered: +org.trimou.handlebars.i18n.ResourceBundleHelper+, +org.trimou.handlebars.NumberIsEvenHelper+, +org.trimou.handlebars.NumberIsOddHelper+ and +org.trimou.handlebars.i18n.DateTimeFormatHelper+. Read javadoc for more info.
...
+org.trimou.gson.resolver.JsonElementResolver+ makes it easier to work with +com.google.gson.JsonElement+. In particular it allows you to access +JsonObject+ properties via dot notation (e.g. if foo is an instance of JsonObject then +foo.bar+ is translated to +foo.get("bar")+), handles +JsonNull+, automatically unwraps +JsonPrimitive+ (e.g. if foo is an instance of JsonPrimitive and contains a boolean value then "foo" is translated to "foo.getAsBoolean()"; this can be disabled - see JsonElementResolver javadoc) and +JsonArray+ elements can be accessed via index (e.g. if foo is an instance of JsonArray then "foo.1" is translated to "foo.get(1)").

NOTE: The JsonElementResolver is automatically loaded if you place the extension jar on the classpath.
