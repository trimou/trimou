...
`JsonNull` is resolved as a `Placeholder#NULL` and `JsonPrimitive` is automatically unwrapped.
E.g. `foo.bar` in `JsonObject` example is translated to `foo.get("bar").getAsNumber()` if bar is an instance of a `java.lang.Number`.
This automatic unwrapping can be also disabled - see `JsonElementResolver` javadoc.

However, unwrapping only works if `JsonElementResolver` is involved!
...
`org.trimou.jsonp.resolver.JsonValueResolver` makes it easier to work with `javax.json.JsonValue` instances. It is automatically loaded if you place the extension jar on the classpath.
